...
class Worker():
    ...
    def __init__(self, name, s_size, a_size, trainer=None, model_path=None, player_mode=False):
        ...
        if not player_mode:
            ...
            self.summary_writer = tf.compat.v1.summary.FileWriter(params.summary_path+"/train_"+str(self.number))
        
        #The Below code is related to setting up the Doom environment  
        self.env, self.actions = create_environment(scenario=params.scenario, no_window=params.no_render, player_mode=player_mode)
        ...
        if params.use_curiosity:
            if params.RND:
                self.local_Pred = RND_curiosity(s_size, a_size, self.name+"_P", trainer, player_mode)
                self.update_local_ops_P = update_target_graph('global_P',self.name+"_P")

            else:
                self.local_Pred = ICM_curiosity(s_size, a_size, self.name+"_P", trainer, player_mode)
                self.update_local_ops_P = update_target_graph('global_P',self.name+"_P")  
    ...
    def initialiaze_game_vars(self):
        ...
        self.last_total_ammo2 = 50  
        self.last_total_kills = 0
        self.last_total_hitc = 0
        self.last_px = 708
        self.last_py = 159
    ...
    def update_containers(self):
        ...
        if params.scenario=='deathmatch':
            ...
            self.episode_hit.append(self.last_total_hitc)
    ...
    def update_summary(self):
        ...
        if params.scenario=='deathmatch':
            ...
            mean_hit = np.mean(self.episode_hit[-params.freq_summary:])
            ...
            summary.value.add(tag='Perf/hitC', simple_value=float(mean_hit))
    ...
    def print_end_episode_perfs(self):
        ...
        """
        if params.scenario=='deathmatch':
            print('{}, kills:{}, hits{}, episode #{}, ep_reward: {}, ep_curiosity: {}, av_reward:{}, av_curiosity:{}, steps:{}, time costs:{}'.format(
                            self.name, self.last_total_kills,self.last_total_hitc, self.episode_count,
        ...
        if params.scenario=='health_gathering_supreme':
    ...
    def get_px_reward(self):
        d = abs(int(self.env.get_game_variable(GameVariable.POSITION_X) - int(self.last_px)))*0.00004
        self.last_px = self.env.get_game_variable(GameVariable.POSITION_X)
        return d

    def get_py_reward(self):
        d = abs(int(self.env.get_game_variable(GameVariable.POSITION_Y) - int(self.last_py)))*0.00004
        self.last_py = self.env.get_game_variable(GameVariable.POSITION_Y)
        return d



    def get_hitcount(self):
        d_kill = self.env.get_game_variable(GameVariable.HITCOUNT) - self.last_total_hitc
        self.last_total_hitc = self.env.get_game_variable(GameVariable.HITCOUNT)
        # if d_f > 0 :
        return float(0.45)*d_kill
    ...
    def get_health_reward(self):
        ...
        if params.scenario == 'health_gathering_supreme':
            dh = self.env.get_game_variable(GameVariable.HEALTH) - self.last_total_health
            self.last_total_health = self.env.get_game_variable(GameVariable.HEALTH)
            if dh>0:
                return 100
            if dh < -10:
    ...
    def get_kill_reward(self):
        ...
        # if d_kill > 0 :
        return float(0.35)*d_kill
        
    
    def get_custom_reward(self,game_reward,rs):
        """
        Description
        --------------
        ...
        if not rs:
            return game_reward
        if params.scenario=='deathmatch':
            a, b, c, x, y = self.get_health_reward(), self.get_kill_reward(), self.get_ammo_reward(), self.get_px_reward(), self.get_py_reward()
            return game_reward + a + b + c + x + y
        
        if params.scenario == 'health_gathering_supreme':
            h = float(self.get_health_reward())
            return h + game_reward
    ...
    def train(self,rollout,sess,gamma,bootstrap_value):
        ...
        # Update the local curiosity network using gradients from loss
        if params.use_curiosity:
            if params.RND:
                feed_dict_P = {self.local_Pred.s1:np.vstack(next_observations)}        
            
                self.L, _ = sess.run([self.local_Pred.loss,
    ...
    def work(self,max_episodes,gamma,sess,coord,saver):
        ...
        total_steps = 0
        self.stdrew = 1
        self.runstd = RunningStats()
        r_shaping = params.reward_shaping
        ...
        with sess.as_default(), sess.graph.as_default():             
            while (not coord.should_stop()) and (self.episode_count<max_episodes):
                ...
                while self.env.is_episode_finished() == False:
                    ...
                    else: reward = self.get_custom_reward(self.env.make_action(self.actions[action_index], 2), r_shaping)
                    ...
                    if params.use_curiosity:
                        if params.RND:
                            curiosity = (np.clip(self.local_Pred.pred_bonus(s1),-1,1))
                            # self.runstd.push(curiosity)
                            # self.oldstd = self.stdrew
                            # self.stdrew = self.runstd.standard_deviation()
                            # if self.stdrew==0:
                            #     self.stdrew = self.oldstd
                            # curiosity = (curiosity - self.runstd.mean())/self.stdrew

                        else:
                            curiosity = (np.clip(self.local_Pred.pred_bonus(s,s1,a_dist[0]),-1,1))*10
                            # self.runstd.push(curiosity)
                            # self.stdrew = self.runstd.standard_deviation()
                            # if self.stdrew==0:
                            #     self.stdrew = 1
                            # curiosity = curiosity/self.stdrew
                    ...
                    if len(episode_buffer) == params.n_steps and done != True and self.episode_step_count != max_episodes - 1:
                        ...
                        if params.use_curiosity:
                            if params.RND:
                                self.v_l,self.p_l,self.e_l,self.L,self.g_n,self.v_n = Losses_grads
                            else: self.v_l,self.p_l,self.e_l,self.Inv_l,self.Forward_l,self.g_n,self.v_n = Losses_grads  
                        else :
                ...
                if len(episode_buffer) != 0:
                    if params.use_curiosity:
                        if params.RND:
                            self.v_l,self.p_l,self.e_l,self.L,self.g_n,self.v_n = self.train(episode_buffer,sess,gamma,0.0) 

                        else: self.v_l,self.p_l,self.e_l,self.Inv_l,self.Forward_l,self.g_n,self.v_n = self.train(episode_buffer,sess,gamma,0.0)
                    else :
    ...
    def play_game(self, sess, episode_num):
        ...
        for i in range(episode_num):
            ...
            while not self.env.is_episode_finished():
                ...
                a_index = self.choose_action_index(a_dist, deterministic=False)
                # Make action and get reward
                reward = self.env.make_action(self.actions[a_index])
                step += 1
                episode_rewards += reward

            print('End episode: {}, Total Reward: {}, {}, items {}'.format(i, episode_rewards, last_total_shaping_reward, self.env.get_game_variable(GameVariable.ITEMCOUNT)))
